...
const redis = require("redis"),
    ...
    db1 = 1,                     //用来存放uid
    db0 = 0,                     //用来存放token
    client = redis.createClient({port:RDS_PORT, host:RDS_HOST,RDS_OPTS,password:RDS_PWD, db : db0,no_ready_check:true}),       //token->uid, 这个用kv
    client2 = redis.createClient({port:RDS_PORT, host:RDS_HOST,RDS_OPTS,password:RDS_PWD, db : db1,no_ready_check:true}),       //uid->token, 这个用hash
    client3 = redis.createClient({port:RDS_PORT, host:RDS_HOST,RDS_OPTS,password:RDS_PWD,db : db15,no_ready_check:true});
    ...
    token : {
        ...
        * */
        setToken:async (user, token) => {
            ...
            let obj = {};
            obj[token] = jwtToken;
            client2.hmset(user, obj, (err, reply) => {
                if(err) throw err;
                else {
                    client2.hgetall(user, (err, obj) => {
                        Object.keys(obj).forEach((value, index) => {
                            if(value != token) {
                                client.del(value, client.print);
                                client2.hdel(user, value, client.print);
                            }
                        });
                    });
                }
